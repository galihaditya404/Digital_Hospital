-- 0. CLEANUP (Drop existing tables to start fresh)
...
-- Profiles (loose link to auth.users to allow seed data)
create table profiles (
  id uuid primary key, -- matches auth.users.id conceptually, but no strict FK to allow dummy seeds
...
BEGIN
  ...
  INSERT INTO doctors (id, user_id, specialization, consultation_fee, is_active)
  VALUES (new_id, new_id, 'Bedah Umum', 500000, true);
...
BEGIN
  ...
  INSERT INTO doctors (id, user_id, specialization, consultation_fee, is_active)
  VALUES (new_id, new_id, 'Spesialis Anak', 350000, true);
...
BEGIN
  ...
  INSERT INTO doctors (id, user_id, specialization, consultation_fee, is_active)
  VALUES (new_id, new_id, 'Spesialis Jantung', 600000, true);
